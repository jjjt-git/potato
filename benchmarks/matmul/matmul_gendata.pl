...
sub printArray
{
  ...
  print "num_t ".$arrayName."[ARRAY_SIZE] = \n";
  print "{\n";
...
sub main()
{
  ...
  for ( my $i = 0; $i < $opts{"size"}; $i++ ) {
    for ( my $j = 0; $j < $opts{"size"}; $j++ ) {
      $mat_values1->[$i][$j] = int(rand(64));
      $mat_values2->[$i][$j] = int(rand(64));
    }
  }
  ...
  print "\n\#define ARRAY_SIZE ".(3 * $opts{"size"}*$opts{"size"})." \n\n";
  #print "\n\#define DIM_SIZE ".$opts{"size"}." \n\n";
  #print "\ntypedef int data_t;";
  
  my @region = (@values1, @values2, @values3);
   
  printArray( "DATA_REGION", \@region );
